...
var cwd = process.cwd();
var rcFile = path.join(cwd, ".php-wasm-rc");
var rcFileExists = fs.existsSync(rcFile);
...
function make(rule, options = [], checkRC = false, spawnWith = { encoding: "utf8" }) {
  return spawnSync(
    "make",
    [
      ...options,
      checkRC && rcFileExists ? `ENV_FILE=${rcFile}` : "",
      rule
    ].filter(Boolean),
    { stdio: "inherit", cwd: `${__dirname}/..`, ...spawnWith }
  );
}
function buildCommand(flags, ...buildArgs) {
  const envName = buildArgs.includes("node") ? "node" : buildArgs.includes("worker") ? "worker" : "web";
...
function helpCommand(flags, command = null) {
  if (command) {
    if (!commands[command]) {
...
function copyAssetsCommand() {
  ...
  const allFiles = ls.stdout.split("\n").map((x) => x || ".").map((dir) => {
    ...
    if (!p.files) return [];
    const files = p.files?.filter((name) => name.match(/\.(so|dat)$/)).map((file) => path.join(dir, file)) || [];
    if (!files) return [];
    return files;
  }).flat();
  ...
  allFiles.forEach((file) => {
    ...
    if (name.slice(0, 3) === "php" && name.slice(3, 3) !== phpVersion) {
      return;
    }
